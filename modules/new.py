...
def download_blueprint(location):
    ...
    urllib.request.urlretrieve("https://api.github.com/repos/CoderMuffin/MuffinStudioAndroidAppBase/zipball", location+".zip", reporthook=lambda c, b, s:
...
def new_project():
    ...
    download_blueprint(location)

    modules.util.info("Specializing blueprint...")
    modules.util.find_and_replace(os.path.join(location, f"app/src/main/java/com/codermuffin/{PLACEHOLDER}/MainActivity.java"), PLACEHOLDER, name.lower())
    ...
    with open(".muffin_studio", "x") as f:
